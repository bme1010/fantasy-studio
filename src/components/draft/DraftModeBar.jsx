...
export default function DraftModeBar({
  ...
  return (
    <div className="mb-2 flex flex-col gap-1.5 rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* Drafted count + undo/reset now share the same row as the mode
            buttons instead of the helper text getting its own row below */}
        <div className="ml-auto flex items-center gap-2 text-xs">
          <span className="font-semibold text-zinc-300">{draftedCount} drafted</span>
          <button
            onClick={undoLast}
            disabled={!canUndo}
            className="rounded-md border border-zinc-700 px-2 py-0.5 text-zinc-300 hover:bg-zinc-800 disabled:opacity-40 disabled:hover:bg-transparent"
          >
            Undo
          </button>
          <button
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-col gap-0.5">
            <input
              ...
              className="w-40 rounded-md border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-white placeholder-zinc-600 focus:border-red-500 focus:outline-none"
            />
          ...
          <div className="flex flex-col gap-0.5">
            <input
              ...
              className="w-20 rounded-md border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-white focus:border-red-500 focus:outline-none"
            />
            <span className="text-[10px] text-zinc-500">Season year</span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-col gap-0.5">
            <input
              ...
              className="w-64 rounded-md border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-white placeholder-zinc-600 focus:border-violet-500 focus:outline-none"
            />
            <span className="text-[10px] text-zinc-500">
...
function SyncStatus({ isSyncing, lastSynced, syncError }) {
  if (isSyncing) return <span className="text-[11px] text-zinc-500">Syncing…</span>;
  if (syncError) return <span className="text-[11px] text-red-400">{syncError}</span>;
  if (lastSynced) {
    return (
      <span className="text-[11px] text-zinc-500">Synced {lastSynced.toLocaleTimeString()}</span>
    );
  }
  return null;
}